...
#' @export
#'
#' @examples # Similar to function PIPET.
...
Group_PIPET <- function(Seurat_data, markers, gene_col=NULL, class_col=NULL, group=NULL,
                        rm_NA=TRUE, freq_counts=NULL, normalize=TRUE, scale=TRUE,
                        nPerm = 1000, distance = "cosine", nCores=4, nslices=4) {
  library(tibble)
  library(Seurat)
  library(parallel)
  library(stats)
  ...
  #### Check Seurat data
  if (!class(Seurat_data) == "Seurat")
    stop("The type of single cell data must be Seurat.")
  ...
  #### Preprocess markers
  keep_gene <- markers$genes %in% rownames(Seurat_data)
  if (sum(keep_gene) > 0) {
  ...
  #### Check group column
  if(is.null(Seurat_data@meta.data[,group]) | is.null(group)){
    stop("Please check if the group column is missing, or try to use the PIPET() function.")
  } else {
    g <- unique(Seurat_data@meta.data[,group])

    message(paste0("Calculating......"))
    f <- function(i, markers, nslices){
      Seurat::Idents(Seurat_data) <- group
      SC <- subset(Seurat_data, idents = g[i] , invert = FALSE)
      counts <- as.data.frame.matrix(SC@assays[["RNA"]]@counts)
      #message(paste0(g[i] ," Start------>"))
      ...
      tmp <-  PIPET(SC_data=counts, markers=markers, gene_col=gene_col, class_col=class_col,
                    rm_NA=rm_NA, freq_counts=freq_counts, normalize=normalize, scale=scale,
                    nPerm = nPerm, distance = distance, nCores=nCores)
      return(tmp)
    }
    ...
    res <- parLapply(cl,1:length(g),f,markers=markers,nslices=nslices)
    stopCluster(cl)
